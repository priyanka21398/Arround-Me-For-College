			...
			<div id="content">
				<div id="sidebar">
					...
					<form action="district_search.php" method="POST">
						<input type="text" name="city" placeholder="Enter the Area"/>
				...
				<div id="main">
					...
					<?php
					$id=intval($_GET['id']);
					$sql="select * from college where cid='$id'";
					$result=mysql_query($sql);
					while($row=mysql_fetch_assoc($result))
					{
					?>
					<h4><?php echo $row['college'] ?></h4>
					<p><div class="title">About Us</div><br/><?php echo $row['about'] ?></p><br/>
					<p>Address: <?php echo $row['address'] ?></p>
						<p>Email: <?php echo $row['email'] ?></p>
						<p>Phone: <?php echo $row['phone'] ?></p>
						<p>Mobile <?php echo $row['mobile'] ?></p>
						<p>Website:&nbsp;<a href="<?php echo $row['website'] ?>" target="_blank"><?php echo $row['website'] ?></a></p>
					<?php	
					}
					?>
